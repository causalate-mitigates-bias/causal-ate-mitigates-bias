...
def compute_training_batch_filtered_using_model_scores_with_length(model, tokenized_loader, device,
...
def create_training_dataloader(training_data, vocab, batch_size=32):
    ...
    outputs = [label if isinstance(label, torch.Tensor) else torch.tensor([label], dtype=torch.float32) for _, label in training_data]
